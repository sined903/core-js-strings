...
function removeFirstOccurrences(str, value) {
  ...
  strToArr.splice(index, value.length);
...
function sumOfCodes(str) {
  ...
  if (typeof str !== 'string' || str.length < 1) {
    ...
    const strToArr = str.split('');
    const toCode = strToArr.map((element) => {
      return element.charCodeAt(0);
    });
    result = toCode.reduce((a, c) => {
      return a + c;
    });
  }
...
function isPalindrome(str) {
  const regex = /[a-z]/gi;
  const chengeStr = str.toLowerCase().match(regex).join('');
  const reversStr = chengeStr.split('').reverse().join('');

  return chengeStr === reversStr;
}
...
function reverseWords(str) {
  const revers = str.split(' ').map((element) => {
...
function invertCase(str) {
  const invert = str.split('').map((element) => {
    let result = '';
...
function extractNameFromTemplate(value) {
  ...
  strToArr.shift();
  const joinArr = strToArr.join(' ');
  return joinArr.slice(0, -1);
}
